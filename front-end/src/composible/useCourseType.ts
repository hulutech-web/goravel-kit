import Api from "@/api"
import {destroy} from "@/api/courseTypeController";
        ...
        proxyConfig: {
            ...
            ajax: {
                ...
                query: ({page, sorts, filters, form}) => {
                    ...
                    return request({
                        url: `/api/admin/course_type?pageSize=${page.pageSize}&currentPage=${page.currentPage
                ...
                save: ({body}) => {
                    ...
                    if (insertRecords.length > 0) {
                        insertRecords.forEach(record => {
                            promises.push(Api.courseTypeController.store(record))
                        })
                    }

                    if (updateRecords.length > 0) {
                        updateRecords.forEach(record => {
                            promises.push(Api.courseTypeController.update({id:record.id}, record))
                        })
                    }
        ...
        columns: [
            ...
            },
            {
                ...
                editRender: {
                    name: 'ASelect',
                    options: [
                        {value: "no_limit", label: "包教会"},
                        {value: "training", label: "课包课"},
