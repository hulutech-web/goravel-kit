        ...
        proxyConfig: {
            ...
            ajax: {
                ...
                query: async ({page, sorts, filters, form}) => {
                    ...
                    return request({
                        url: `/api/admin/notice?pageSize=${page.pageSize}&currentPage=${page.currentPage
                ...
                save: ({body}) => {
                    ...
                    if (insertRecords.length > 0) {
                        insertRecords.forEach(record => {
                            promises.push(Api.noticeController.store(record))
                        })
                    }

                    if (updateRecords.length > 0) {
                        updateRecords.forEach(record => {
                            promises.push(Api.noticeController.update({id: record.id}, record))
                        })
                    }
