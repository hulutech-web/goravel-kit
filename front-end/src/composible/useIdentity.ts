...
export default () => {
    ...
    const fillOptions = async () => {
        ...
        let row1 = gridOptions.columns.find(item => item.field === "venue_id")
        if (row1) {
            row1.editRender.options = venueList.value
        }
    }
    // 表单配置
        ...
        proxyConfig: {
            ...
            ajax: {
                ...
                query: ({page, sorts, filters, form}) => {
                    ...
                    return request({
                        url: `/api/admin/identity?pageSize=${page.pageSize}&currentPage=${page.currentPage
                ...
                save: ({body}) => {
                    ...
                    if (insertRecords.length > 0) {
                        insertRecords.forEach(record => {
                            promises.push(Api.identityController.store(record))
                        })
                    }

                    if (updateRecords.length > 0) {
                        updateRecords.forEach(record => {
                            promises.push(Api.identityController.update({id: record.id}, record))
                        })
                    }
